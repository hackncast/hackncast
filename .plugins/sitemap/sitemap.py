...
class SitemapGenerator(object):
    ...
    def __init__(self, context, settings, path, theme, output_path, *null):
        ...
        self.ignore = settings.get('SITEMAP_IGNORE', [])
        ...
        if not isinstance(config, dict):
            ...
            valid_chfreqs = ('always', 'hourly', 'daily', 'weekly', 'monthly',
                    'yearly', 'never')
            valid_ignore = ('tags', 'categories', 'authors')
            ...
            for ign in ignore:
                if ign not in valid_ignore:
                    warning("sitemap plugin: invalid ignore `{0}'".format(ign))
                else:
    ...
    def generate_output(self, writer):
        ...
        pages = self.context['pages'] + self.context['articles']
        if 'categories' not in self.ignore: 
            pages += [ c for (c, a) in self.context['categories']]
        if 'tags' not in self.ignore: 
            pages += [ t for (t, a) in self.context['tags']]
        if 'authors' not in self.ignore: 
